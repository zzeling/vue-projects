<template>
    <div>
        ...
        <div  v-show="orderList!=''" class="flex flex-items-center flex-content-between titBg">
            ...
            <div style="width:40%;">下单时间</div>
            <div style="width:20%;">数量</div>
            <div style="width:20%;">订单状态</div>
        </div>
        <div v-for=" list in orderList" :key="list.id" @click="MarketBuy(list.orderId)">
            <div style="height:60px;text-align:left;" class="flex   padding-r-l flex-items-center flex-content-between">
                ...
                <div style="color:#929292;width:40%;" v-if="list.placeOrderTime!=null||list.placeOrderTime!=undefined">{{list.placeOrderTime.substr(5,11)}}</div>
...
export default {
    ...
    methods:{
         pagechange:function(current){
            Indicator.open();
            this.$http.post('app/c2cTrade/getMyTradeOrder?pageNo='+current+'&pageSize='+this.pageSize,{orderType:1}).then((data)=>{
                Indicator.close();
